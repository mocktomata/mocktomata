...
- 4f3639e: Adopt current first-party majors: `standard-log-color@^13.2.0`, `async-fp@^10.0.0`, `clibuilder@^11.0.0`, and `@unional/fixture@^5.0.0`.
...
- Updated dependencies [4f3639e]
  - @mocktomata/framework@10.1.0
  - @mocktomata/nodejs@10.1.0
  - @mocktomata/io-remote@10.1.0
  - @mocktomata/service@10.1.0
...
- 98560f3: Depend on `type-plus` `8.0.0-beta.10` exactly, up from `^7.0.0`.
  ...
  **Minor**, not major: `type-plus` is a runtime dependency here (`ts/browser/context.ts` uses
  `requiredDeep` and `RequiredPick`) but it does not leak into the published declarations — no
  emitted `.d.ts` in this package imports from `type-plus`, so consumers do not inherit
  `type-plus@8`'s new `typescript >= 5.6.0` peer through this package's own types. The last
  published `mocktomata` (9.2.3) declares `type-plus: ^7.0.0`, so that is the jump consumers see.
