...
describe(`maskValue(string)`, () => {
	...
	incubator(
		'log (and spec record) does not contain masked value',
		{ logLevel: logLevels.all },
		(specName, spec, reporter) => {
			test(specName, async () => {
				spec.maskValue('secret')
	...
	incubator.sequence(
		'invoke returns new masked value during simulate',
		{ logLevel: logLevels.all },
	...
	incubator.sequence(
		'get returns masked value during simulate',
		{ logLevel: logLevels.all },
		(specName, { save, simulate }, reporter) => {
			test(specName, async () => {
	...
	incubator.sequence(
		'get returns new masked value during simulate',
		{ logLevel: logLevels.all },
		(specName, { save, simulate }, reporter) => {
			test(specName, async () => {
				{
	...
	incubator(
		'invoke returns input in array gets original value',
		{ logLevel: logLevels.all },
		(specName, spec, reporter) => {
			test(specName, async () => {
...
describe(`maskValue(regex)`, () => {
	...
	incubator.sequence(
		'masks the matched value greedy',
		{ logLevel: logLevels.all },
		(specName, { save, simulate }, reporter) => {
			test(specName, async () => {
				{
