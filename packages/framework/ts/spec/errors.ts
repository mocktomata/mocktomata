...
export class ExtraReference extends MocktomataError {
  constructor(public specName: string, public subject: any, options?: ModuleError.Options) {
    super(
      `Recorded data for '${specName}' does not expect a new reference to be created: ${tersify(subject, { maxLength: 50 })}`,
      options)
  }
}
...
export class ExtraAction extends MocktomataError {
  constructor(public specName: string, public state: Recorder.State, public actionId: number, public action: SpecRecord.Action, options?: ModuleError.Options) {
    super(
...
export class MissingAction extends MocktomataError {
  ...
  ${prettifyAction(state, actionId, action)}`, options)
  }
}

export class ActionMismatch extends MocktomataError {
  constructor(public specName: string, public actual: MismatchActionModel | undefined, public expected: MismatchActionModel | undefined, options?: ModuleError.Options) {
    super(`Recorded data for '${specName}' doesn't match with simulation.
Expecting action:
${tersifyAction(expected)}
Received:
${tersifyAction(actual)}`, options)
  }
}
