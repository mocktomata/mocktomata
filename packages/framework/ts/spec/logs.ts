...
export function logCreateSpy({ log }: LogContext, { ref, refId }: Pick<Recorder.State, 'ref' | 'refId'>, maskCriteria: MaskCriterion[], profile: SpecRecord.SubjectProfile, subject: any) {
...
export function logAction({ log }: LogContext, state: Recorder.State, actionId: SpecRecord.ActionId, action: SpecRecord.Action) {
  log.on(logLevels.debug, () => prettifyAction(state, actionId, action))
}

// istanbul ignore next
export function logRecordingTimeout({ log }: LogContext, specName: string, timeout: number) {
  log.warn(`${specName}: done() was not called (in ${timeout} ms). Did the test takes longer than expected or you forget to call done()?`)
}

export function logCreateStub({ log }: LogContext, { ref, refId }: Pick<Recorder.State, 'ref' | 'refId'>, profile: SpecRecord.SubjectProfile, subjectOrMeta: any) {
  log.on(logLevels.debug, log => log(`${ref.plugin} <ref:${refId}> create ${profile} stub: ${tersify(subjectOrMeta)}`))
}

// istanbul ignore next
export function logMissingResultAction({ log }: LogContext, state: Recorder.State, actionId: SpecRecord.ActionId, action: SpecRecord.Action) {
