...
import { createTestContext, DuplicatePlugin, loadConfig, loadPlugins, PluginModuleNotConforming, PluginNotConforming, PluginNotFound } from '../index.js'
...
test('Not existing plugin throws PluginNotFound', async () => {
  ...
  await a.throws(() => context
    .extend(loadConfig).extend(loadPlugins).get(), PluginNotFound)
})
...
it('throws PluginModuleNotConforming when the plugin missing activate function', async () => {
  ...
  await a.throws(() => context.extend(loadConfig).extend(loadPlugins).get(), PluginModuleNotConforming)
})
...
test('plugin missing support method throws', async () => {
  ...
  await a.throws(() => context.extend(loadConfig).extend(loadPlugins).get(), PluginNotConforming)
})
...
test('registering plugin with the same name throws DuplicatePlugin', async () => {
  ...
  await a.throws(() => context.extend(loadConfig).extend(loadPlugins).get(), DuplicatePlugin)
})
