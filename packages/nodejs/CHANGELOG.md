...
- 98560f3: Depend on `type-plus` `8.0.0-beta.10` exactly, up from `^7.0.0`.
  ...
  **Minor**, not major: `type-plus` is a runtime dependency here (`ts/io.ts` uses `reduceByKey`)
  but it does not leak into the published declarations — no emitted `.d.ts` in this package
  imports from `type-plus`, so consumers do not inherit `type-plus@8`'s new
  `typescript >= 5.6.0` peer through this package's own types. The last published
  `@mocktomata/nodejs` (9.2.4) declares `type-plus: ^7.0.0`, so that is the jump consumers see.
