...
export function getEffectiveSpecModeContext(mode?: Spec.Mode) {
	return mode
		? { mode }
		: (ctx: { config: Spec.Context['config']; specName: string; specRelativePath: string }) => ({
				mode: getEffectiveSpecMode(ctx.config, ctx.specName, ctx.specRelativePath)
			})
}
 ...
 */
export function getEffectiveSpecMode(config: Spec.Context['config'], specName: string, invokePath: string): Spec.Mode {
