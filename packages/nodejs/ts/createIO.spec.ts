...
describe(`${createIO.name}()`, () => {
	...
	describe(`loadConfig()`, () => {
		...
		it('merges two or more configs and emits a warning', async () => {
			...
			expect(reporter.getLogMessages()).toEqual([
...
- mocktomata.json`
	...
	describe(`loadPlugin()`, () => {
		...
		it.only('loads cjs plugin', async () => {
		...
		it('throws PluginNotFound when package have not activate export', async () => {
			...
			a.satisfies(
				stderr,
				/PluginModuleNotConforming: @mocktomata\/plugin-fixture-no-activate is not a valid plugin module./
			)
		})
		...
		it('throws PluginNotFound when package activate is not a fuction', async () => {
			...
			a.satisfies(
				stderr,
				/PluginModuleNotConforming: @mocktomata\/plugin-fixture-activate-not-fn is not a valid plugin module./
			)
		})
	})
